#!/usr/bin/env bash
  ...
  run_user_delete() {
    ...
    if [[ "$YN_ANSWER" == "y" ]]; then
      ...
      CLEAN_CMD=(find "$BASE" -xdev -type d -empty -mindepth 1)
      for xd in "${EXCLUDE_DIRS[@]}"; do CLEAN_CMD+=( ! -path "$xd" ! -path "$xd/*" ); done
      CLEAN_CMD+=( -delete )
  ...
  while true; do
    ...
    case "$MODE" in
      0)
        ...
        echo "  M) Delete MULTIPLE files (enter one full path per line)"
        echo "  B) Back to main menu"
        echo "------------------------------------------"
        read -r -p "Enter choice (S/M/B): " SUBCH
        [[ -n "${SUBCH:-}" ]] || { msg_no_input; continue; }
        ...
        case "${SUBCH^^}" in
          ...
          M)
            ...
            if [[ ${#MULTI_FILES[@]} -eq 0 ]]; then
              echo "ERROR: No file paths were entered. Returning to main menu..."
              echo ""
              continue
            fi
            ...
            for mf in "${MULTI_FILES[@]}"; do
              if [[ -f "$mf" ]]; then
                printf "%-65s %s\n" "$mf" "VALID"
                VALID_FILES+=("$mf")
              else
                printf "%-65s %s\n" "$mf" "INVALID / NOT FOUND"
                INVALID_FILES+=("$mf")
              fi
            done
            echo ""
            echo "Summary: Total entered=${#MULTI_FILES[@]}  Valid=${#VALID_FILES[@]}  Invalid=${#INVALID_FILES[@]}"
            echo ""

            if [[ ${#VALID_FILES[@]} -eq 0 ]]; then
              echo "ERROR: No valid files to delete. Returning to main menu..."
              echo ""
              continue
            fi

            if [[ ${#INVALID_FILES[@]} -gt 0 ]]; then
              echo "WARNING: Some entries were invalid and will be SKIPPED:"
              for bf in "${INVALID_FILES[@]}"; do echo "  - $bf"; done
              echo ""
              echo "Choose next step:"
              echo "  R) Re-enter the file list"
              echo "  C) Continue with the ${#VALID_FILES[@]} valid file(s) only"
              echo "  A) Abort to main menu"
              read -r -p "Enter choice (R/C/A): " MCH
              [[ -n "${MCH:-}" ]] || { msg_no_input; continue; }
              case "${MCH^^}" in
                R) echo "Re-enter requested. Returning to main menu..."; echo ""; continue ;;
                C) echo "Continuing with ${#VALID_FILES[@]} valid file(s)."; echo "" ;;
                A) msg_abort; continue ;;
                *) echo "Invalid choice. Returning to main menu..."; echo ""; continue ;;
              esac
            fi
            ...
            echo "  $MULTI_LOG"
            echo ""
            echo "Preview of files to be deleted (first 20):"
            head -n 20 "$MULTI_LOG" | sed 's/^/  /'
            if [[ ${#VALID_FILES[@]} -gt 20 ]]; then
              echo "  ... ($((${#VALID_FILES[@]} - 20)) more files)"
            fi
            echo ""

            echo "FINAL CONFIRMATION (two-step):"
            echo "1) Review the full list in: $MULTI_LOG"
            echo "2) Type 'CONFIRM' to acknowledge the list is correct"
            echo "3) Then type 'YES' to execute deletion"
            echo ""

            read -r -p "Type 'CONFIRM' to acknowledge candidate list (or anything else to abort): " MACK
            if [[ "${MACK:-}" != "CONFIRM" ]]; then
              echo "Deletion aborted by operator after review."
              echo ""
              continue
            fi
            ...
            M_DELETED=0
            M_FAILED=0
            M_FAILED_LOG="$(mktemp "/tmp/isi_multi_failed_${SAFE_REQ}.XXXX")"

            if [[ "${WORM_CHECK_MODE^^}" == "STRICT" ]]; then
              echo "STRICT mode: prechecking WORM state using isi worm files view (may be slow)..."
              for mf in "${VALID_FILES[@]}"; do
                mstate="$(worm_state_of "$mf")"
                if [[ "$mstate" == "COMMITTED" ]]; then
                  if isi worm files delete -f "$mf"; then
                    echo "Deleted: $mf"
                    ((M_DELETED++)) || true
                  else
                    echo "ERROR: Delete failed for: $mf"
                    ((M_FAILED++)) || true
                    printf '%s\t%s\n' "$mf" "DELETE_FAILED" >> "$M_FAILED_LOG"
                  fi
                else
                  echo "SKIPPED (not COMMITTED, state=$mstate): $mf"
                  ((M_FAILED++)) || true
                  printf '%s\t%s\n' "$mf" "$mstate" >> "$M_FAILED_LOG"
                fi
              done
            else
              echo "FAST mode: attempting delete directly; failures are logged."
              for mf in "${VALID_FILES[@]}"; do
                if isi worm files delete -f "$mf"; then
                  ...
                  echo "ERROR: Delete failed for: $mf"
                  ((M_FAILED++)) || true
                  printf '%s\t%s\n' "$mf" "DELETE_FAILED_OR_NOT_COMMITTED" >> "$M_FAILED_LOG"
                fi
              done
            fi

            echo ""
            echo "Multi-file deletion complete. Deleted=$M_DELETED Failed=$M_FAILED"
            if [[ "$M_FAILED" -gt 0 ]]; then
              echo "Failures saved to: $M_FAILED_LOG"
              echo "Showing first 20 failures (path<TAB>reason/state):"
              head -n 20 "$M_FAILED_LOG" | sed 's/^/  /'
            fi
            echo ""
            ;;

          B)
            echo "Returning to main menu..."; echo ""
            continue
            ;;

          *)
            echo "WARNING: Invalid sub-menu choice [$SUBCH]. Returning to main menu..."
            echo ""
            continue
            ;;
        esac
      ...
      4)
        ...
        ;;
      *)
        echo "WARNING: Invalid choice. Please select 0/1/2/3/4."
        echo ""
        ;;
    esac
  done
